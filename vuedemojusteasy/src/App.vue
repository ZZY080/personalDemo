    ...
    <el-table ref="singleTable" :data="tableData" highlight-current-row @current-change="handleCurrentChange"
      ...
      <el-table-column property="url" label="url">
        <template slot-scope="scope">
         <div> <a :href="scope.row.url" target="_blank"  @click="handleClick(scope.row.id)">{{ scope.row.url }}</a></div>
        </template>
      </el-table-column>
      <el-table-column property="iserror" label="有bug?">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" v-if="!scope.row.iserror"
            @click="handleError(scope.row.id)" ref="confirm">确认有bug</el-button>
          <el-tag type="danger" v-else ref="confirm">有bug</el-tag>
        </template>
      </el-table-column>
      <el-table-column property="isfinsh" label="完成?">
        <template slot-scope="scope">
          <!-- 在这里添加自定义内容 -->
          <el-button type="primary" size="mini" v-if="!scope.row.isfinsh"
            @click="handleFinsh(scope.row.id)" >确认完成</el-button>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      const { status, data} = (await this.$http.get(`/geterror?page=${page}&perPage=${perPage}`)).data;
      if (status == 200) {
    ...
    async handleFinsh(id) {
      ...
      await this.$http.get(`/cancelerror?id=${id}`);
      this.getData();
    },
